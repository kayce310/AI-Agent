...
- Khi vá UI callback, ưu tiên API MATLAB phổ biến `drawnow` thay vì option phụ thuộc version.
- Nếu playback/slider có độ trễ, tránh `drawnow limitrate` trong vòng lặp animation vì cơ chế throttle có thể trì hoãn flush UI event; dùng `drawnow` thường và để callback/timer kiểm soát pacing.
