#!/usr/bin/env node
/**
 * @file state-manager-bridge — CLI bridge for KatoStateManager
 * @layer core
 * @depends-on src/core/memory/state-manager.ts
 * @owner core-memory
 *
 * Provides CLI access to KatoStateManager for scripts/ layer.
 * This bridge allows scripts/kato-state-manager.ts to delegate
 * state operations without importing from core/ directly (R2 rule).
 *
 * Usage: npx tsx src/core/cli/state-manager-bridge.ts <command> [args...]
 */

import { KatoStateManager, ProcessedFile } from '../memory/state-manager.js';

const [, , command = 'read', ...args] = process.argv;
const manager = new KatoStateManager();

type ActionResult = { ok: true; [key: string]: unknown } | { ok: false; error: { code: string; message: string; details?: unknown } };

async function runCommand(commandName: string, commandArgs: string[]): Promise<ActionResult> {
  switch (commandName) {
    case 'init':
